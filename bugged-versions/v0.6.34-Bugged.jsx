...
(function() {
    ...
    // Since paragraph text is clipped to the textbox boundaries, we can't measure its full height directly.
    // Instead, we create a duplicate as point text (which is not clipped) and reduce its font size until
    // its full rendered height is less than or equal to the textbox height.
    function adjustTextToFit(layer, textboxHeight) {
        ...
        // Position duplicate at same location.
        dupLayer.textItem.position = layer.textItem.position;
        
        // Measure the duplicate's full rendered height.
        var dupBounds = dupLayer.bounds;
        var dupHeight = dupBounds[3].as("px") - dupBounds[1].as("px");
        var iterations = 0;
        // Iteratively reduce font size until the duplicate's height fits.
        ...
        // Instead of copying dupLayer.textItem.leading (which might be in auto-leading mode),
        // we read the final font size and then set the original's leading to that value.
        var finalSize = parseFloat(dupLayer.textItem.size.as("px"));
        layer.textItem.size = new UnitValue(finalSize, "px");
        layer.textItem.leading = new UnitValue(finalSize, "px");
