...
// 9. Increases the text’s font size as much as possible (using a tolerance based on the text’s internal offset)
...
(function() {
    ...
    // Because the rendered text may have an internal offset (i.e. its bounds.top might not equal textY),
    // we compute an offset and then check only the bottom boundary.
    function maximizeFontSizeToFillTextBox(layer, containerX, containerY, containerWidth, containerHeight) {
        ...
        // Adjust the container bottom using this offset.
        var adjustedContainerBottom = containerY + containerHeight + offsetY;
        ...
        while (true) {
            ...
            // Force Photoshop to update the text layer (a short pause may be needed on slower machines)
            // You can also try: app.refresh();
            var b = layer.bounds;
    ...
    // That is, desiredSmartHeight = (origHeight - marginVert) - textY.
