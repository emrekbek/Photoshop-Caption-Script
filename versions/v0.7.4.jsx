...
// 9. Increases the text’s font size as much as possible so that it fills the textbox without overflowing.
...
(function() {
    ...
    // ---------- NEW STEP: Increase the font size as much as possible
    // so that the text fills the text box without overflowing.
    function maximizeFontSizeToFillTextBox(layer, containerLeft, containerTop, containerWidth, containerHeight) {
        ...
        while (true) {
            ...
            // Get the rendered bounds of the text layer.
            ...
            // Check if the text overflows the designated container.
            // (container boundaries: left = containerLeft, top = containerTop,
            //  right = containerLeft + containerWidth, bottom = containerTop + containerHeight)
            if (right > containerLeft + containerWidth || bottom > containerTop + containerHeight) {
                // Overflow detected: revert to the previous font size and exit.
                layer.textItem.size = new UnitValue(currentSize, "px");
                break;
            }
            // Otherwise, update the current size and try a larger size.
            currentSize = currentSize + 1;
        }
    }

    // Maximize the font size so the text fills the text box as much as possible.
    ...
    // (Since the text box’s top is at textY, the desired new height is:
    //    desiredSmartHeight = (canvas height - marginVert) - textY = origHeight - 2×marginVert)
